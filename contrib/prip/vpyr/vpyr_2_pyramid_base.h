...
template <class D> struct vmap_simple_data {
...
template <class D> struct vmap_replicated_data {
  typedef D value_type;
  std::vector<value_type> d;
  vmap_replicated_data() {
    // d.push_back(this->ReturnType()) ; INJ
  }
  value_type &data(vmap_level_index arg_level) { return d[arg_level]; }
...
public:
  ...
  //: The last level.
  vmap_level_index last_level() const { return last_level_ / 2; }
  ...
  bool modified_at_last_level() const { return !(last_level_ & 1); }
  ...
  bool is_pendant(vmap_level_index level) const { return this == sigma(level); }
...
template <class D> class vmap_2_pd_dart : public vpyr_2_pyramid_base_dart {
public:
